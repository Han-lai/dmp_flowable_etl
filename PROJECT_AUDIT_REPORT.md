...
#### 2. 抽取層 (Extract Layer)
- **工具**: Python Script (`scripts/etl/sync_unified.py`) + JDBC Bridge。
- **邏輯**: 
...
| :--- | :--- | :--- | :--- |
| **scripts/etl/** | Sync Engine | 負責資料同步 (MSSQL -> Bronze) 與系統重建 | 依賴 `config/` 中的 JDBC 設定 |
| **sql/etl/** | Data Warehouse | 定義 Bronze / Silver / Gold 層的 Table Schema 與 ELT 邏輯 | 被 `execute_etl.py` 呼叫執行 |
...
### A. 資料同步 (Data Sync)
- **入口點**: `scripts/etl/sync_unified.py`
- **主要功能**: 
...
- **Gold L5 完成率**: `sql/etl/06_gold_kpi_task_completion.sql`
...
整個 Extract-Transform-Load 流程由 `scripts/etl/sync_unified.py` 統一調度，採用 **Direct JDBC Insertion** 模式，由 ClickHouse 直接對 MSSQL 發起查詢，不經過 Python 記憶體，確保大數據量傳輸效率。
...
#### 2. L5 任務完成率 & 狀態判斷
實作於 `sql/etl/04_silver_fact_tasks.sql` 與 `06_gold_kpi_task_completion.sql`。
...
| 檔案名稱 | 層級 | 功能說明 | 類型 |
|:---|:---|:---|:---|
| `sql/etl/01_bronze_flowable_core.sql` | Bronze | Flowable 核心表 (Task/Var/Proc) | `CREATE TABLE` |
...
- **存取點**: VM IP `REDACTED_IP`，API 外部監聽埠位 `7088`。
