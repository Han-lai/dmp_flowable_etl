...
def verify_core_rule():
    ...
    for i, target_date in enumerate(dates):
        ...
        SELECT 
            '{target_date}' as Date,
            (SELECT COUNT(*) FROM CoreUsers) as CoreActive,
            {expected[i]} as Expected
        """
        
        df = pd.read_sql(query, conn)
        results.append(df)
        match = "MATCH" if df.iloc[0]['CoreActive'] == expected[i] else f"DIFF: {df.iloc[0]['CoreActive'] - expected[i]}"
        print(f"Date {target_date}: CoreActive={df.iloc[0]['CoreActive']}, Expected={expected[i]} -> {match}")
    ...
    # Add error summary
    diff = final_df['CoreActive'] - final_df['Expected']
    print(f"\nTotal Absolute Error: {diff.abs().sum()}")
