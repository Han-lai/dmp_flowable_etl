...
# Include the 'E' and 'C' task exclusions that Silver utilizes
excl = """
AND NOT EXISTS (
...
def process_scope(title, plant, factory, line, region, vx_type, out_file):
    ...
    with open(out_file, 'a', encoding='utf-8') as f:
        f.write(f"### {title}\n")
        if has_diff:
            f.write("| 日期 | 不符欄位 | 真實重構 QAS 預期值 | 重新建立之 ClickHouse Gold | 差異 |\n")
            f.write("|---|---|---|---|---|\n")
            for r in diff_rows:
                f.write(r + "\n")
        else:
...
def main():
    outfile = 'output_final_verification_report.md'
    with open(outfile, 'w', encoding='utf-8') as f:
        f.write("## 最終修復驗證報告 (MDM & Vx Logic 雙重修復後)\n\n")

    process_scope("V1 / CNS / DG3 / SMT / ST02", "DG3", "SMT", "ST02", "CNS", "V1", outfile)
    process_scope("V3 / CNS / DG3 / SMT / ST02", "DG3", "SMT", "ST02", "CNS", "V3", outfile)
    process_scope("V3 / CNE / WJ2 / NBU / E5", "WJ2", "NBU", "E5", "CNE", "V3", outfile)
    process_scope("V1 / CNE / WJ2 / NPE / NPE3", "WJ2", "NPE", "NPE3", "CNE", "V1", outfile)
