...
# QAS 採用「正確的業務規則」：廠區特權 > MO前綴 > TaskKey
vx_logic = """
CASE 
...
def process_scope(title, plant, factory, line, region, vx_type):
    ...
    with open('output_diff_after_mdm_fix.md', 'a', encoding='utf-8') as f:
        f.write(f"### {title}\n")
        if has_diff:
            f.write(f"> QAS 採用規則：廠區特權 > 工單前綴 > TaskKey<br>\n")
            f.write(f"> CH 現有規則：TaskKey > 工單前綴\n\n")
...
def main():
    with open('output_diff_after_mdm_fix.md', 'w', encoding='utf-8') as f:
        f.write("## 統一邏輯驗證差異報告 (MDM 修復後，尚未套用 Vx 邏輯修正)\n\n")
    ...
    print("Done. Wrote output_diff_after_mdm_fix.md")
