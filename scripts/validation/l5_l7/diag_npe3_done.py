...
def diag_npe3():
    ...
    CASE 
        ...
        WHEN (v_factory.TEXT_ LIKE '%NPE%' OR v_plant.TEXT_ LIKE '%NPE%') 
             AND LEFT(v_mo.TEXT_, 3) IN ('196','199','200','210','212','213','315') THEN 'V1' -- Notice 369 is excluded!
    ...
    # 分析 12-26 的 V1 Done (這天有些許資料)
    df_done_v1_26 = df_v1[
        ((df_v1['start_date'] == '2025-12-26') | (df_v1['claim_date'] == '2025-12-26') | (df_v1['end_date'] == '2025-12-26')) &
        (df_v1['end_date'].notna() & (df_v1['end_date'] <= '2025-12-26'))
    ]
    if len(df_done_v1_26) > 0:
        print(f"\n--- 12-26 V1 Done (QAS={len(df_done_v1_26)}) 任務名稱分佈 ---")
        print(df_done_v1_26['NAME_'].value_counts().to_string())
    ...
    # 計算 12-25 的 V3 來看看因為剔除 369 後，哪些單子掉下來了
    df_done_v3_25 = df_v3[
