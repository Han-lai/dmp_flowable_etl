...
def main():
    ...
    print("=== L5 Indicators Verification (WJ2 NBA N5 - Multi-Vx) ===")
    print("Calculated from Silver Layer (Point-in-time & Rolling Acc)")
    print()
    print("| Date | Vx | Total | TODO | DOING | DONE | Doing+Done | Todo+Doing(Acc) |")
    print("|------|----|------:|-----:|------:|-----:|-----------:|----------------:|")

    for d in dates:
        # Calculate daily and acc metrics for all Vx on N5
        query = f"""
            SELECT 
                vx_type,
