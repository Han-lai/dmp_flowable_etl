# ClickHouse L5 Benchmark Automation Tool

This document provides a **complete benchmark automation tool** for
testing query performance on ClickHouse using the official
`clickhouse-benchmark` CLI.

The script automates:

1.  SQL generation for benchmark queries
2.  Multi-stage concurrency testing
3.  Warm-up execution
4.  Parsing benchmark results
5.  Generating reports (TXT + Markdown)
6.  Saving raw benchmark logs
7.  Generating performance charts
...
# Architecture
    ...
    │
    ├─ Phase1  Generate queries.sql
    │
    ├─ Phase2  Warmup benchmark
    │
    ├─ Phase3  Concurrency benchmark
    │        ├─ 10 users
    │        ├─ 50 users
    │        └─ 100 users
    │
    ├─ Phase4  Parse metrics
    │
    ├─ Phase5  Generate reports
    │        ├─ raw logs
    │        ├─ txt report
    │        ├─ markdown report
    │        └─ performance chart
...
-   ClickHouse server
-   clickhouse-benchmark installed
-   Python 3.8+
...
# Benchmark Configuration
  ...
  -------------------- -----------------------------------
  CONCURRENCY_LEVELS   Simulated concurrent users
  ITERATIONS           Total benchmark queries per stage
  WARMUP_ITERATIONS    Warmup runs to warm cache
...
-   ClickHouse POC evaluation
-   BI dashboard performance testing
-   Superset workload simulation
