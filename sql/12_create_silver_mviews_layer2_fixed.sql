-- ========================================
-- Silver 層 Materialized Views - 第二層（業務邏輯層）- 修正版本
-- ========================================
-- 修正問題：日期過濾邏輯與 MSSQL 不一致
-- MSSQL 使用: (START_TIME_ BETWEEN ... OR CLAIM_TIME_ BETWEEN ... OR END_TIME_ BETWEEN ...)
-- 修正時間: 2026-01-23 15:30:00
...
-- ========================================
-- 修正日期邏輯：不在 MVIEW 層做日期過濾，而是在查詢時過濾
...
CREATE VIEW silver.vw_l5_metrics_mssql_compatible AS
SELECT 
    vx_type,
    vx_subtype,
    plant,
    ...
    -- 使用與 MSSQL 一致的日期過濾邏輯
    SUM(total_task_qty) AS total_task_qty,
    SUM(todo_qty) AS todo_qty,
    SUM(doing_qty) AS doing_qty,
    SUM(done_qty) AS done_qty,
    SUM(excluded_qty) AS excluded_qty,
    SUM(bypass_qty) AS bypass_qty,
    SUM(special_v1_rule_qty) AS special_v1_rule_qty,
    
    MAX(_mview_update_time) AS _mview_update_time
...
-- 建立完成提示
-- ========================================
SELECT 'Silver Layer 2 Fixed MViews Created Successfully' AS status,
       'Fixed date filtering logic to match MSSQL OR logic' AS fix_description,
       'MVIEW tables: mv_fact_task_vx_attribution, mv_l5_metrics_realtime' AS created_tables,
       'Query views: vw_fact_task_vx_attribution_realtime, vw_l5_metrics_mssql_compatible' AS created_views;