...
def execute_varinst_batch_sync(client, batch_id: str, start_time: str, end_time: str) -> int:
    """執行單個 ACT_HI_VARINST_0108 批次同步"""
    logger.info(f"同步批次: {batch_id}")
    logger.info(f"時間範圍: {start_time} ~ {end_time}")
    
    # 先清理可能存在的舊資料
    cleanup_sql = f"""
    ALTER TABLE bronze.bpm_act_hi_varinst DELETE 
    WHERE _batch_id = '{batch_id}'
       OR (CREATE_TIME_ >= '{start_time}' AND CREATE_TIME_ < '{end_time}')
    """
    ...
    FROM jdbc('mssql_master', '
        SELECT * FROM APP_SRV_BPM.dbo.ACT_HI_VARINST_0108
        WHERE CREATE_TIME_ >= ''{start_time}'' 
          AND CREATE_TIME_ < ''{end_time}''
          AND CREATE_TIME_ IS NOT NULL
...
def sync_varinst_batches(limit: int = None):
    """同步 ACT_HI_VARINST_0108 批次"""
    ...
    try:
        with session_controller.get_stateless_session() as client:
            ...
            # 取得所有 running 狀態的 ACT_HI_VARINST_0108 批次
            running_sql = """
            SELECT 
                batch_id,
                watermark_start,
            ...
            for i, (batch_id, start_time, end_time) in enumerate(result.result_rows, 1):
                ...
                try:
                    ...
                    status_manager.update_batch_status(
                        'ACT_HI_VARINST_0108', batch_id, BatchStatus.RUNNING
                    )
                    
                    # 執行同步
                    row_count, duration = execute_varinst_batch_sync(
                        client, batch_id, start_time, end_time
                    )
                    
                    # 更新為完成狀態
                    status_manager.update_batch_status(
                        'ACT_HI_VARINST_0108', batch_id, BatchStatus.COMPLETED,
                        row_count, duration
                    )
                    ...
                    try:
                        status_manager.update_batch_status(
                            'ACT_HI_VARINST_0108', batch_id, BatchStatus.FAILED,
                            0, 0, str(e)
                        )
                    except:
                        pass
...
def main():
    ...
    parser = argparse.ArgumentParser(description='同步 ACT_HI_VARINST_0108 批次')
    ...
    try:
        sync_varinst_batches(limit=args.limit)
