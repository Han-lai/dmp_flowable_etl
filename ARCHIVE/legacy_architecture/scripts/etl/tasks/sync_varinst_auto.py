#!/usr/bin/env python3
"""
Auto sync ACT_HI_VARINST_0108 batches - 5 batches per round, 1 minute rest
"""
...
def get_remaining_batches():
    """取得剩餘待同步的批次數量"""
    try:
        script_path = os.path.join(os.path.dirname(__file__), 'sync_varinst_batches.py')
...
def sync_batch_group(batch_size=5):
    """同步一組批次"""
    try:
...
def main():
    ...
    logger.info("Starting auto sync of ACT_HI_VARINST_0108 batches")
    logger.info("Strategy: 5 batches per round, 1 minute rest between rounds")
