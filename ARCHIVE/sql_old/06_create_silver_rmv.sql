-- ============================================
-- Silver Layer Refreshable Materialized Views (RMV)
-- 使用 ClickHouse 24.3+ 的定時刷新功能
-- 命名規則: RMV_{表名} (Refreshable Materialized View)
-- 已對齊 View 定義 (sql/05_create_silver_views.sql)
-- ============================================

-- ============================================
-- 0. RMV_PROC_VARIABLES_PIVOTED - 流程變數樞紐表
-- Grain: 一個 PROC_INST_ID = 一列
-- 刷新頻率: 每天
-- 對齊: V_PROC_VARIABLES_PIVOTED
-- ============================================
DROP TABLE IF EXISTS silver.RMV_PROC_VARIABLES_PIVOTED;
...
-- ============================================
-- 0.1 RMV_TASK_VARIABLES_PIVOTED - 任務變數樞紐表 (新增)
-- Grain: 一個 TASK_ID = 一列
-- 刷新頻率: 每天
-- 對齊: V_TASK_VARIABLES_PIVOTED
-- ============================================
DROP TABLE IF EXISTS silver.RMV_TASK_VARIABLES_PIVOTED;
...
-- ============================================
-- 1. RMV_HI_PROC_TASK_NODE - 任務節點表
-- Grain: 一個 Task ID = 一列
-- 刷新頻率: 每天
-- 對齊: V_HI_PROC_TASK_NODE
-- ============================================
DROP TABLE IF EXISTS silver.RMV_HI_PROC_TASK_NODE;
...
-- ============================================
-- 2. RMV_HI_PROCINST_NODE - 流程實例表
-- Grain: 一個 PROC_INST_ID = 一列
-- 刷新頻率: 每天
-- 對齊: V_HI_PROCINST_NODE
-- ============================================
DROP TABLE IF EXISTS silver.RMV_HI_PROCINST_NODE;
CREATE MATERIALIZED VIEW silver.RMV_HI_PROCINST_NODE
REFRESH EVERY 1 DAY
ENGINE = ReplacingMergeTree()
ORDER BY PROC_INST_ID
SETTINGS allow_nullable_key = 1
...
LEFT JOIN silver.RMV_PROC_VARIABLES_PIVOTED AS v ON p.PROC_INST_ID_ = v.PROC_INST_ID
LEFT JOIN bronze.bpm_act_re_procdef AS pd ON p.PROC_DEF_ID_ = pd.ID_;


-- ============================================
-- 3. RMV_HI_BIZ_EVENT_INFO - 業務事件表
-- Grain: 一個 BUSINESS_KEY = 一列
-- 刷新頻率: 每天
-- 對齊: V_HI_BIZ_EVENT_INFO (已完全等價)
-- ============================================
DROP TABLE IF EXISTS silver.RMV_HI_BIZ_EVENT_INFO;
CREATE MATERIALIZED VIEW silver.RMV_HI_BIZ_EVENT_INFO
REFRESH EVERY 1 DAY
ENGINE = ReplacingMergeTree()
