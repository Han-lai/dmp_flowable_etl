...
METRICS = {
    '1': ('在途業務事件總數', """
        SELECT count(*) AS CNT FROM silver.RMV_HI_BIZ_EVENT_INFO WHERE FINAL_END_TIME IS NULL
    """),
    '2': ('在途任務總數', """
        SELECT sum(TASK_TODO_CNT + TASK_DOING_CNT) AS CNT FROM silver.RMV_HI_BIZ_EVENT_INFO WHERE FINAL_END_TIME IS NULL
    """),
    '3': ('事件自動完成率', """
        SELECT 
    ...
    """),
    '4': ('TASK_STATUS 分布', """
        SELECT TASK_STATUS, count(*) AS cnt FROM silver.RMV_HI_PROC_TASK_NODE GROUP BY TASK_STATUS ORDER BY cnt DESC
    """),
    '5': ('在途任務數 - 依廠區', """
        SELECT coalesce(PLANT, 'Unknown') AS PLANT, count(*) AS CNT 
        FROM silver.RMV_HI_PROC_TASK_NODE WHERE TASK_STATUS IN ('TODO', 'DOING') 
    ...
    """),
    '8': ('平均業務事件總歷時 (秒)', """
        SELECT round(avg(TOTAL_DURATION_SEC), 2) AS AVG_SEC FROM silver.RMV_HI_BIZ_EVENT_INFO WHERE FINAL_END_TIME IS NOT NULL
    """),
    '9': ('平均任務處理時長 (秒)', """
        SELECT round(avg(WORK_DURATION_SEC), 2) AS AVG_SEC FROM silver.RMV_HI_PROC_TASK_NODE WHERE TASK_STATUS = 'DONE'
    """),
    '10': ('在途流程健康度快照 (Top 10)', """
        SELECT coalesce(FIRST_PROC_DEF_NAME, 'Unknown') AS PROC_NAME, count(*) AS CNT 
        FROM silver.RMV_HI_BIZ_EVENT_INFO WHERE FINAL_END_TIME IS NULL 
    ...
    '11': ('依流程的自動完成率 (Top 10)', """
        ...
        FROM silver.RMV_HI_PROC_TASK_NODE WHERE TASK_STATUS IN ('DONE', 'DONE_AUTO')
