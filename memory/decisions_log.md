...
### 2026-01-13: ClickHouse 原生增量 MView JOIN 行為驗證

**決策**：確認不使用 ClickHouse 原生增量 MView（TO table 語法）

**驗證方式**：建立測試腳本 `scripts/test_imv_join_behavior.py`
...
**結論**：
- ClickHouse 原生增量 MView 只監控主表（FROM 子句）
- JOIN 表的變更不會觸發已寫入資料的更新
- 因為 11 個指標都需要 JOIN 維度表（部門、廠區、流程名稱），選擇全量刷新確保資料一致性
- 即使未來維度表不再變更，因為表與表 JOIN 關係複雜，仍建議使用全量刷新
