...
| taskCreateTime | START_TIME_ | task_start_date | snapshot_date | Date | 格式轉換 (toDate) | ✅ 正常 |
| taskClaimTime | CLAIM_TIME_ | task_claim_date | snapshot_date | Date | 格式轉換 (toDate) | ✅ 正常 |
| taskEndTime | END_TIME_ | task_end_date | snapshot_date | Date | 格式轉換 (toDate) | ✅ 正常 |
| - | - | vx_type | vx_type | String | 業務邏輯計算 | ✅ 正常 |
| - | - | is_excluded | is_excluded | Int | 排除邏輯 (自動對齊) | ✅ 正常 |
| - | - | exclude_reason | exclude_reason | String | 排除原因 | ✅ 正常 |
...
### 核心 MVIEW: `silver.mv_fact_task_vx` (2026-02-03 修復版)

```sql
FROM bronze.bpm_act_hi_taskinst t
LEFT JOIN silver.mv_varinst_pivoted v
    ON t.PROC_INST_ID_ = v.PROC_INST_ID_                    -- 1:1 關係 (已修復碎片化)
LEFT JOIN silver.mv_dim_mfg_five_level mdm 
    ON v.varinst_lineName = mdm.line_name                  -- 1:1 關係 (MDM 整合)
LEFT JOIN bronze.common_hr_employee he
    ON t.ASSIGNEE_ = he.EmpCode                             -- 1:1 關係
LEFT JOIN bronze.bpm_act_hi_varinst tb
    ON t.ID_ = tb.TASK_ID_ AND tb.NAME_ = 'autoComplete'    -- 1:1 關係
```

**架構突破**:
1. **多時間維度**: 取代單一 `snapshot_date`，改用 `ARRAY JOIN [start, claim, end]` 展開，完美對齊 QAS SQL 邏輯。
2. **Refreshable Pivot**: 解決 `ReplacingMergeTree` 導致的「維度覆蓋」問題，確保 Plant/Line 不遺失。
3. **MDM 修復**: 切換至 `common_mdm_mfg_plant_master` 路徑，解決 90% 以上的 Plant Code 缺失。
...
## 📈 基準數據核帳快照 (Verification Benchmarks)

以下為核心指標在邏輯修正後的最終驗證數值，供未來開發與維護查驗。

### 1. WJ2 E5 核心對話 (12/25 基準)
*   **Total Task (Done)**: 192 ✅ (100% 對齊 MSSQL)
*   **Acc Qty (Todo+Doing)**: 40 ✅ (100% 對齊用戶預期)

### 2. 累積比率修正驗證 (WJ2 E5 12/28 週末)
此數據驗證了「7天滾動分母」邏輯是否成功消除了週末暴衝。
*   **Acc Qty**: 47
*   **Total Task (當日)**: 11
*   **7D Rolling Total (分母)**: 724
*   **Acc Rate (修正後)**: **6.49%** ✅ (對應目標約 7%)
*   **之前報錯值**: 427.27% (分母誤採當日量)

### 3. CNS DG3 區域驗證 (12/31)
此數據驗證了模型在多區域下標籤與路徑的準確性。
*   **標籤對應**: S06 $\rightarrow$ ST06 (廠區: SMT)
*   **Total Task**: 208
*   **Done Task**: 115
*   **Acc Qty**: 489
*   **7D Rolling Total**: 2,964
*   **Acc Rate**: **16.50%** ✅
