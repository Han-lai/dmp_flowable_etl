...
| 表名 | 用途 | 記錄數 | 關鍵欄位 |
|------|------|--------|---------|
| silver.mv_varinst_pivoted | EAV 轉置表 (Refreshable) | 17,949 | varinst_plant, varinst_factory, varinst_lineName |
...
1. **MVIEW 手動/自動更新**：Silver 層 L2 MVIEW 自動跟隨 Bronze INSERT。
2. **Pivot/Gold 層刷新**：每小時自動刷新 (`REFRESH EVERY 1 HOUR`)。
3. **即時查詢**：Cube 層支援即時查詢，資料一致性偏差為 0。
