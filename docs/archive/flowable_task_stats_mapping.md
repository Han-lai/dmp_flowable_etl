...
**版本：** 1.0  
**建立日期：** 2026-01-30  
**資料來源：** `APP_SRV_COMMON.dbo.FlowableTaskStats_0202` (MSSQL 直連，不需同步至 ClickHouse)
**備註：** 本文件定義的驗證邏輯，應直接對 MSSQL 來源表執行查詢驗證。
...
## 📝 L5 指標 SQL 查詢範例 (MSSQL T-SQL)

以下 SQL 查詢可直接從 `APP_SRV_COMMON.dbo.FlowableTaskStats_0202` 生成 L5 任務指標。
**注意：** 由於直接查詢 MSSQL，語法已調整為 T-SQL。
...
SELECT 
    ...
    -- 任務數統計
    count(*) AS total_task,
    sum(case when upper(TaskStatus) = 'TODO' then 1 else 0 end) AS todo_count,
    sum(case when upper(TaskStatus) = 'DOING' then 1 else 0 end) AS doing_count,
    sum(case when upper(TaskStatus) = 'DONE' then 1 else 0 end) AS done_count,
    
    -- 完成率與執行率
    cast(round(sum(case when upper(TaskStatus) = 'DONE' then 1 else 0 end) * 100.0 / count(*), 2) as decimal(10,2)) AS completion_rate,
    cast(round(sum(case when upper(TaskStatus) IN ('DOING', 'DONE') then 1 else 0 end) * 100.0 / count(*), 2) as decimal(10,2)) AS execution_rate
...
WHERE 
    ...
    AND (
        ...
        OR CAST(TaskClaimTime as DATE) = '2025-12-25'
