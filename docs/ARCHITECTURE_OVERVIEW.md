...
└─────────────────────────────────────────────────────────────────────┘
                                  ...
                                  ▼ Python 同步腳本 (增量/全量)
┌─────────────────────────────────────────────────────────────────────┐
│  Bronze 層 (原始資料)           ClickHouse REDACTED_IP:8121      │
│  ┌─────────────────────────────────────────────────────────────────┐│
...
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ Layer 1:                                                        ││
│  │ • mv_varinst_pivoted      (EAV → 寬表轉置)                      ││
│  │ • mv_dim_mfg_five_level   (五階維度主檔)                        ││
│  │                                                                 ││
│  │ Layer 2:                                                        ││
...
| 表名 | 來源 | 更新機制 | 說明 |
|------|------|---------|------|
| `mv_varinst_pivoted` | bpm_act_hi_varinst | POPULATE | EAV 轉置 |
| `mv_dim_mfg_five_level` | common_mdm_* | POPULATE | 五階維度 |
| `mv_fact_task_vx` | 上述表 JOIN | POPULATE | 核心事實表 |
...
| `sql/rebuild/02_silver_layer1.sql` | Silver L1 (透視表 + 維度) |
| `sql/rebuild/03_silver_layer2.sql` | Silver L2 (核心事實表) |
| `sql/rebuild/04_gold_refreshable.sql` | Gold 層 (Refreshable MView) |
| `sql/rebuild/05_cleanup_path_a.sql` | 清理舊架構 |
| `sql/rebuild/06_validation.sql` | 驗證查詢 |
